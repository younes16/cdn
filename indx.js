...
$('#qbuy_switch').change(function() {
	if ($(this).prop('checked')) {
		...
		$('.device-qbuy').css({'filter':"grayscale(0)","-webkit-filter":"grayscale(0)","-moz-filter":"grayscale(0)"});
	}
	else {
		$('.qbuy_position').attr('disabled','true');
		$('.device-qbuy').css({'filter':"grayscale(100%)","-webkit-filter":"grayscale(100%)","-moz-filter":"grayscale(100%)"});
	}
});
...
$('.rate_modal-review').text('Review Now!').css({'width':'90%','float':'none','margin':'0 auto'}).removeClass('add_review').addClass('review_redirect');
$('.rate_mod-body').text('Would you mind taking a moment to review us on Shopify\'s App Store? Each review helps us tremendously!â€¨');
});
...
function apply_settings() {
  ...
  $('.aph_add_cart').css({'background':'#'+ $('.buttonColor').val(), 'color':'#'+ $('.textColor').val()});
